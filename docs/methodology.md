...
CNN/GRU early stopping uses the configured validation partition. Other quick models use fixed, documented settings and do not make a random internal validation split; no hyperparameter choice is made from the final test partition. The quick configuration records one deterministic seed; `full_benchmark.yaml` records seeds 11, 42, and 73 as separate rows and writes mean/sample-standard-deviation aggregates without selecting a winner from the locked test set.
